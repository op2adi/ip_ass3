...
with open('Q6.txt','a') as f:
    rty={str(((D-C).total_seconds())):'Dictionary',str(((B-A).total_seconds())):'OOPS'}
    PRTY={str((F-E).total_seconds()):'OOPS',str(((H-G).total_seconds())):'dICTIONARY'}
    f.write('Grading operation'+'\n')
    f.write(str('N=996')+'\n'+f'Time taken by oops {str((B-A).total_seconds())}'+'\n'+f'Time taken by dictionary {(D-C).total_seconds()}'+'\n'+f'Faster Dictionary'+'\n'+f'Fraction: {str(max((D-C).total_seconds(),(B-A).total_seconds())-min((D-C).total_seconds(),(D-C).total_seconds()))}'+'\n')
    f.write('SEARCHING DATA'+'\n')
    f.write(str('N=996')+'\n'+f'Time taken by oops {str((H-G).total_seconds())}'+'\n'+f'Time taken by dictionary {str((F-E).total_seconds())}'+'\n'+f'Faster Dictionary'+'\n'+f'Fraction: {str(max((F-E).total_seconds(),(H-G).total_seconds())-min((F-E).total_seconds(),(H-G).total_seconds()))}'+'\n')
